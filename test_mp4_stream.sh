...
echo "Capturing 10 seconds of MP4 stream to /tmp/test_stream.mp4..."
timeout 10 curl -s "$BASE_URL/stream.mp4" > /tmp/test_stream.mp4 || true

FILE_SIZE=$(stat -f%z /tmp/test_stream.mp4 2>/dev/null || stat -c%s /tmp/test_stream.mp4 2>/dev/null)
...
FILE_TYPE=$(file /tmp/test_stream.mp4 2>/dev/null || echo "unknown")
...
if echo "$FILE_TYPE" | grep -qi "ISO Media\|MP4\|video"; then
    ...
    xxd -l 100 /tmp/test_stream.mp4 || hexdump -C /tmp/test_stream.mp4 | head -10
fi
echo ""

# Step 6: Check for MP4 boxes
echo "[6/6] Checking for MP4 structure..."
if xxd /tmp/test_stream.mp4 2>/dev/null | head -5 | grep -q "ftyp"; then
    echo "✓ Found 'ftyp' box (file type)"
else
    echo "WARNING: No 'ftyp' box found"
fi

if xxd /tmp/test_stream.mp4 2>/dev/null | head -50 | grep -q "moov"; then
    echo "✓ Found 'moov' box (movie header)"
else
    echo "WARNING: No 'moov' box found"
fi

if xxd /tmp/test_stream.mp4 2>/dev/null | grep -q "mdat"; then
...
echo "==========================================="
echo ""
echo "File saved to: /tmp/test_stream.mp4"
echo "File size: $FILE_SIZE bytes"
echo ""
echo "Next steps:"
echo "1. Try playing the file with VLC:"
echo "   vlc /tmp/test_stream.mp4"
echo ""
echo "2. Analyze the file with ffprobe:"
echo "   ffprobe /tmp/test_stream.mp4"
echo ""
